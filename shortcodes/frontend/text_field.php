...
function gpg_pub_text_field_shortcode($atts = array()) {
  ...
  $string = '';
  $string .= '<div class="builder-only" style="display:none;">Text Field</div>';
  if (is_singular('pub')) {
      ...
      while( $the_query->have_posts() ) {
        ...
        if ($approved_entry_array) {
          ...
          if ($pub_text_object) {
            ...
            if ($heading) {
              $string .= '<' . $heading_elm . '>' . $heading . '</' . $heading_elm . '>';
            }

            $string .= '<p>' . $before . $pub_text_object[0]->meta_value . $after . '</p>';
